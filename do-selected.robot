...
Log fájlok összefűzése
    ...
    ${CURRENT_LOG_DIR}=    Run Keyword If    ${EXISTS}    Get File    ${CURRENT_LOG_DIR_FILE}    ELSE    Set Variable    MISSING_LOG_DIR
    ...
    IF    ${log_output_dir_exists}
        ...
        Run Keyword If    '${rebot_result}' == 'None'    Log    [HIBÁZOTT] rebot_result None! A rebot processz nem indult el vagy hibás paramétereket kapott.
        ...
        Run Keyword If    not ${MERGED_LOG_EXISTS} and '${rebot_result}' != 'None'    Log    [HIBÁZOTT] mergedlog.html NEM jött létre! stdout: ${rebot_result.stdout} stderr: ${rebot_result.stderr}
...
Robot futtatása
  ...
  IF    ${AUTO_LAUNCH_START_BAT} == True
      IF    ${WORKFLOW_STATUS} == 'READY_TO_RUN'    
          ...
          Set Suite Variable    ${CURRENT_LOG_DIR}
          # Írjuk ki a current_log_dir.txt fájlba is a log könyvtár nevét
          ${CURRENT_LOG_DIR_FILE}=    Set Variable    ${LOG_FILES}/current_log_dir.txt
          Log    [FUTTATÁS] CURRENT_LOG_DIR_FILE path: ${CURRENT_LOG_DIR_FILE}=>${CURRENT_LOG_DIR}
          Create File    ${CURRENT_LOG_DIR_FILE}    ${CURRENT_LOG_DIR}    encoding=UTF-8
          Log     [FUTTATÁS] Új log könyvtár létrehozva: ${CURRENT_LOG_DIR}
